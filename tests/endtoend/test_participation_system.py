...
from environments.stable_democracy.initialization import initialize_stable_democracy_graph_state # Using new init
...
class TestStableParticipationSystem(unittest.TestCase):
    ...
    def _create_stable_test_config(
        ...
        return create_stable_democracy_config(
            ...
            delegate_participation_rate=participation_config_override.delegate_participation_rate if participation_config_override else 0.95,
            voter_participation_rate=participation_config_override.voter_participation_rate if participation_config_override else 0.65,
            temporal_correlation_strength=participation_config_override.temporal_correlation_strength if participation_config_override else 0.0,
            use_locked_values=locked_value_config_override.use_locked_values if locked_value_config_override else True,
            locked_crop_yields=locked_value_config_override.locked_crop_yields if locked_value_config_override else [1.0] * 3,
            eliminate_prediction_noise=locked_value_config_override.eliminate_prediction_noise if locked_value_config_override else True,
            num_crops=len(locked_value_config_override.locked_crop_yields) if locked_value_config_override and locked_value_config_override.locked_crop_yields else 3,
            num_portfolios=3
        )
    ...
    def test_temporal_correlation_participation(self):
        ...
        if len(agent_0_participation_seq) > 1:
            ...
            print(f"  P(P|P) = {prob_p_given_p:.3f} (expected high > {base_rate + (1-base_rate)*correlation_strength*0.5})") # Heuristic check
            print(f"  P(P|NP) = {prob_p_given_np:.3f} (expected low < {base_rate - base_rate*correlation_strength*0.5})") # Heuristic check
    ...
    def test_locked_value_determinism_and_no_noise(self):
        ...
        config = self._create_stable_test_config(
            ...
            seed=303,
            num_crops=len(locked_yields_expected) # Match num_crops to locked_yields
        )
        state = self._initialize_test_state(config)
    ...
    def test_locked_values_with_noisy_perception(self):
        ...
        config_noisy_perception = create_stable_democracy_config(
            mechanism="PDD",
            num_agents=num_agents, 
            locked_value_config_override=locked_conf, 
            seed=505,
            num_crops=len(locked_yields_expected),
            eliminate_prediction_noise=False # Passed to factory
        )
