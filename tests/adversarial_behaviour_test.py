...
class TestAdversarialBehaviorVerification(unittest.TestCase):
    ...
    def test_comprehensive_behavioral_analysis(self):
        ...
        for mechanism, results in report["test_results"].items():
            if "error" not in results:
                print(f"\n{mechanism.upper()}:")
                if "adversarial_stats" in results:
                    print(f"  Adversarial agents chose worst option: {results['adversarial_stats']['chose_worst_option_rate']:.2f}")
                    print(f"  Aligned agents chose worst option: {results['aligned_stats']['chose_worst_option_rate']:.2f}")
