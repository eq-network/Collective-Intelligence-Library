...
class GraphState:
    ...
    def update_node_attrs(
        self, attr_name: str, new_values: jnp.ndarray
    ) -> "GraphState":
        """
    ...
    def split_rng(self) -> Tuple["GraphState", Optional[jnp.ndarray]]:
        """
        Split the RNG key and return updated state plus the split key.

        Returns:
            Tuple of (new_state_with_updated_key, split_key)
        """
        rng_key = self.global_attrs.get("rng_key")
        if rng_key is None:
            return self, None

        new_key, split_key = jrandom.split(rng_key)
        new_state = self.update_global_attr("rng_key", new_key)
        return new_state, split_key
